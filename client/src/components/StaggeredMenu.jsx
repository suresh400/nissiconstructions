  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#fafafa', '#eaeaea'];
          let arr = [...raw];
          if (arr.length >= 3) {
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <a href="#home" className="logo-container logo-container-staggered" onClick={(e) => handleItemClick(e, '#home')} style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none', pointerEvents: 'auto' }}>
          <div style={{
            ...
            borderColor: open ? 'var(--accent-gold)' : 'var(--white)'
          }}>
            N
          </div>
            ...
            {items && items.length ? (
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  <a className="sm-panel-item" href={it.link} onClick={(e) => handleItemClick(e, it.link)} aria-label={t(it.ariaLabel)} data-index={idx + 1}>
                    <span className="sm-panel-itemLabel">{t(it.label)}</span>
                  </a>
                </li>
              ))
            ) : (
              <li className="sm-panel-itemWrap" aria-hidden="true">
                <span className="sm-panel-item">
                  <span className="sm-panel-itemLabel">{t('No items')}</span>
                </span>
              </li>
            )}
          </ul>
